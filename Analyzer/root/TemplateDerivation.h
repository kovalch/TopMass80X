...
private:
  ...
  TTree* modifiedTree_(TChain *tree, int minComboType = -10, int maxComboType = 10, bool isData = false);
  void UnknownChannelAbort();

  void fillAlpha(std::vector<RooFormulaVar*>& alpha, int& h, RooArgSet argSet, std::string add = "");
  std::vector<RooDataSet*> createDataSets(std::vector<double>masses, std::vector<double>JESes, const RooArgSet& varSet);
