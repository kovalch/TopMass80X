...
void FindParametersMass(int iMass)
{
  ...
  if (!plotByMass) {
	...
	case 2: {
        ...
        break;
		  ...
		  case 6: {
        h098 = FindParameters("/nfs/dust/cms/user/mseidel/trees_paper/Summer12_TTJetsMS1785_0.98", 0);
        h100 = FindParameters("/nfs/dust/cms/user/mseidel/trees_paper/Summer12_TTJetsMS1785_1.00", 1);
        h102 = FindParameters("/nfs/dust/cms/user/mseidel/trees_paper/Summer12_TTJetsMS1785_1.02", 2);
        break;
      }
    }
  }
    ...
    h098 = FindParameters("/nfs/dust/cms/user/mseidel/trees/Summer12_TTJets1725_0.98", 0);
    h100 = FindParameters("/nfs/dust/cms/user/mseidel/trees/Summer12_TTJets1725_1.00", 1);
    h102 = FindParameters("/nfs/dust/cms/user/mseidel/trees/Summer12_TTJets1725_1.02", 2);
 ...
 if(nJES==5) h104->Draw("SAME");
  ...
  if(nJES==5)   leg0->AddEntry( h096, "JSF = 0.96", "PL");
    ...
    leg0->AddEntry( h098, "m_{t,gen} = 172.5 GeV, JSF -4%", "PL");
    leg0->AddEntry( h100, "m_{t,gen} = 172.5 GeV", "PL");
    leg0->AddEntry( h102, "m_{t,gen} = 172.5 GeV, JSF +4%", "PL");
    if (obs==0) leg0->AddEntry( h178, "m_{t,gen} = 175.5 GeV", "PL");
  }
  
  leg0->Draw();

  CMS_lumi(cObservable, 4, 0., true, "Private Work");
 if(nJES==5){ 
  ...
  if (obs==0) DrawCutLine(172.5, h098->GetMaximum()*1.);
  else DrawCutLine(80.4, h098->GetMaximum()*1.);
  h098->GetYaxis()->SetRangeUser(0, h098->GetMaximum()*1.5);
 }
