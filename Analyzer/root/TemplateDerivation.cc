...
{
	std::string fileName = str(boost::format("Summer12_TTJetsMS%1%%2%_%3$3.2f") % (int)mass % (int)((mass-int(mass))*10) % jes);
...
std::vector<RooDataSet*> TemplateDerivation::createDataSets(std::vector<double>masses, std::vector<double>JESes, const RooArgSet& varSet)
{
	...
	for(auto mass : masses) {
		for (auto jes : JESes) {
			...
			std::cout << " (nFiles: "
					<< chain->Add(samplePath_ + TString(fileName)) << ") "; // << std::endl;
			tmpFile->cd();
			...
			dataset.push_back(new RooDataSet(name, name, varSet,
					RooFit::Import(*tree), RooFit::WeightVar("combinedWeight")));
			//if(binnedTemplates){
...
void
TemplateDerivation::rooFitTopMass_()
{
	...
	const int nComboTypes = (channelID_ == kAllJets) ? 2 : 3; // number of different permutation types, e.g., correct, rest
	...
	std::vector<double> massValues = {166.5, 169.5, 171.5, 172.5, 173.5, 175.5, 178.5};
	...
	std::vector<RooDataSet*> dataset = createDataSets(massValues, jesValues, varSet);
	...
	if(doCalibration_){
	    ...
	    if(templType == 1 && comboType == 0){
				...
				for(unsigned t=0; t<nTemplates; ++t) {
					workspace[0]->var("JES_" +templ[t])->setVal(jesValues [iTemplateJES [t]]);
					workspace[0]->var("mTop_"+templ[t])->setVal(massValues[iTemplateMass[t]]);
...
TTree*
TemplateDerivation::modifiedTree_(TChain *tree, int minComboType, int maxComboType, bool isData)
{
...
void TemplateDerivation::fillAlpha(std::vector<RooFormulaVar*>& alpha, int& h, RooArgSet argSet, std::string add){
