...
TTree* modifiedTree(TTree * tree, int comboType);
TTree* modifiedTree(TTree * tree, int minComboType, int maxComboType);
TTree* modifiedTree(TTree * tree, int minComboType, int maxComboType, bool isData);
...
void rooFitTopMass()
{
  ...
  for(int templType = 0; templType < nTemplTypes; ++templType){
	...
	comboRangeName = "R5";
      ...
      const unsigned nAlpha = (templType == 0 && minComboType == 1 && maxComboType == 1) ? 2 : ((templType == 1) ? 3 : 4);
      const unsigned n4ParVars = (templType == 0 && minComboType != 1 && maxComboType != 1) ? 2 : 1;
	...
	if (minComboType == 1 && maxComboType == 1){ // mTop, correct
	  ...
	  double a[] = {172.321, 0.988343, 86.304, 0.82852,
			  0.0418171*172.321};
	  ...
	  double a[] = {182.033, 0.273944,  44.5262,  0.747527,
			172.627, 1.07992 , 100.467 , -0.0678561,
			  0.132935*182.033,
			 -0.0506823*(-172.627)};
	  iniPar = std::vector<double>(a,a+sizeof(a)/sizeof(double));
	}
	  ...
	  double a[] = {178.158, 0.273002, 37.4066, 0.495892,
			174.492, 1.0028  , 89.2252, 0.906987,
			  0.123006*178.158,
			 -0.051721*(-174.492)};
	  iniPar = std::vector<double>(a,a+sizeof(a)/sizeof(double));
	}
	...
	if (minComboType == 1 && maxComboType == 1){ // mW, correct
	  ...
	  double a[] = {84.5106, -0.00935425, 41.8388, 0.0809444,
			-0.0507648*(-84.5106),
			-0.0639401*(-84.5106)};
	  iniPar = std::vector<double>(a,a+sizeof(a)/sizeof(double));
	}
	  ...
	  double a[] = {84.089, 0.00523783, 18.2724, 0.150474,
			-0.0512301*(-84.089),
			-0.0742137*(-84.089)};
	  iniPar = std::vector<double>(a,a+sizeof(a)/sizeof(double));
	}
	  ...
	  double a[] = {84.6583, -0.00193411, 9.5101, 0.0125416,
			 0.0565317*84.6583,
			 0.0834242*84.6583};
	  iniPar = std::vector<double>(a,a+sizeof(a)/sizeof(double));
	}                                                                                                                                                     
	...
	if(i == 0 || (i == 1 && templType == 0 && minComboType != 1 && maxComboType != 1)){
	  if(h==0){
	  ...
	  int ref = 0;
	  if((templType == 0 && i == 3 && minComboType != 1 && maxComboType != 1)) ref = 1;
	  alpha[h][i] = new RooFormulaVar(name, name, "@0",
					  //RooArgSet(*par[h][nPar-nAlpha+i], *alpha[h][ref]));
	                                  RooArgSet(*par[h][nPar-nAlpha+i]));
	}
      }

      //name = "workspace_"; name += h;
      //workspace[h] = new RooWorkspace(name, name);
	...
	if(minComboType == 1 && maxComboType == 1) {
	  ...
	  RooRealVar* ratio = new RooRealVar(varName, varName, 0.820546, 0.0, 1.0);
	  ratio->setConstant(kTRUE);
	  varName = "landau_"; varName += h;
	  RooLandau*   landau   = new RooLandau  (varName, varName, *var, *alpha[h][0], *alpha[h][2]);
	  //varName = "gaussian_"; varName += h;
	  //RooGaussian* gaussian = new RooGaussian(varName, varName, *var, *alpha[h][1], *alpha[h][3]);
	  varName = "width_"; varName += h;
	  RooRealVar* VoigtWidth = new RooRealVar(varName, varName, 2.0);
	  VoigtWidth->setConstant(kTRUE);
	  varName = "voigt_"; varName += h;
	  RooVoigtian* voigt = new RooVoigtian(varName, varName, *var, *alpha[h][1], *VoigtWidth, *alpha[h][3]);
	  RooAddPdf* add = new RooAddPdf(name, name, *landau, *voigt, *ratio);
	  workspace[0]->import(*add);
	}
	else if(minComboType == 5 && maxComboType == 5) {
	  TString varName = "ratio_"; varName += h;
	  RooRealVar* ratio = new RooRealVar(varName, varName, 0.758690, 0.0, 1.0);
	  ratio->setConstant(kTRUE);
	  varName = "landau_"; varName += h;
	  RooLandau*   landau   = new RooLandau  (varName, varName, *var, *alpha[h][0], *alpha[h][2]);
	  //varName = "gaussian_"; varName += h;
	...
	vars << workspace[0]->var(varName)->getVal();
	if(i != nPar-1) vars << ", ";
      }
      std::cout << vars.str();
      std::cout << std::endl;
      ...
      if(templType == 0){
	if     (minComboType == 1 && maxComboType == 1)
	  std::cout << "172.321, 0.988343, 86.304, 0.82852, 0.0418171";
	else if(minComboType == 2 && maxComboType == 4)
	  std::cout << "182.033, 0.273944, 44.5262, 0.747527, 172.627, 1.07992, 100.467, -0.0678561, 0.132935, -0.0506823";
	else if(minComboType == 5 && maxComboType == 5)
	  std::cout << "178.158, 0.273002, 37.4066, 0.495892, 174.492, 1.0028, 89.2252, 0.906987, 0.123006, -0.051721";
      }
      else if(templType == 1){
	if     (minComboType == 1 && maxComboType == 1)
	  std::cout << "84.5106, -0.00935425, 41.8388, 0.0809444, -0.0507648, -0.0639401";
	else if(minComboType == 2 && maxComboType == 4)
	  std::cout << "84.089, 0.00523783, 18.2724, 0.150474, -0.0512301, -0.0742137";
	else if(minComboType == 5 && maxComboType == 5)
	  std::cout << "84.6583, -0.00193411, 9.5101, 0.0125416, 0.0565317, 0.0834242";
      }
      std::cout << std::endl;
      //}
    }
  ...
  TTree* tree = modifiedTree(oldTree, -10, 10, isData);
  ...
  for(int templType = 0; templType < nTemplTypes; ++templType){
    for(int comboType = 0; comboType < nComboTypes; ++comboType){
      ...
      //const unsigned nPar   = 4 + nAlpha-1;
      const unsigned nAlpha = (templType == 0 && comboType == 0) ? 2 : ((templType == 1) ? 3 : 4);
      const unsigned n4ParVars = (templType == 0 && comboType != 0) ? 2 : 1;
      const unsigned nPar   = 4*n4ParVars + nAlpha-n4ParVars;
      for(unsigned p=0; p<nPar; ++p) {
	name = "par_"; name += h; name += "_"; name += p;
	workspace[0]->var(name)->setConstant(kTRUE);
	//workspace[0]->var(name)->removeError();
	//workspace[0]->var(name)->removeAsymError();
      }
    }
  }
  ...
  //RooRealVar fSig = RooRealVar("fSig", "fSig", 1);
...
TTree*
modifiedTree(TTree * tree, int minComboType, int maxComboType, bool isData)
{
...
TTree* modifiedTree(TTree * tree, int minComboType, int maxComboType)
{
  return modifiedTree(tree, minComboType, maxComboType, false);
}
TTree* modifiedTree(TTree * tree, int comboType)
{
  return modifiedTree(tree, comboType, comboType);
}
TTree* modifiedTree(TTree * tree)
{
  return modifiedTree(tree, -1, 10);
}
