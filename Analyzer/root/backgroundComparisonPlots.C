...
void backgroundComparisonPlots()
{
  TH1::SetDefaultSumw2();
  gStyle->SetOptStat(0);
  ...
  TString plotString[] = {//"topMasses[0]>>hMT_data"                            , "(probs[0])"
			 ...
			 "topMasses[0]>>hMT2_data"                           , ""
			 ,"(w1Mass[0]+w2Mass[0])/2.0>>hMW2_data"              , ""
			 ,"probs[0]>>hPr2_data"                               , ""
			 //,"TMath::Abs(jets[0].Eta()-jets[1].Eta())>>hDh2_data", ""
			 //,"TMath::Abs(jets[0].Eta()+jets[1].Eta())>>hSh2_data", ""
			 //,"Max$(jets.Pt())>>hPt12_data"                     , ""
			 ,"Njet>>hNjet2_data"                               , ""
  ...
  */
  TFile * tmpFile = TFile::Open("tmpFileComparisonValidaiton.root","RECREATE");
  tmpFile->cd();
...
*/
  std::cout << "Creating BKG FROM SIGNAL" << std::endl;

  TTree * tree = 0;
  TString fileName = "QCDEstimationMix_SIGNAL_2011_NEW_skimmed2_SMALL.root";
  TFile * file = TFile::Open(samplePath+TString(fileName));
  TTree * oriTree = (TTree*)file->Get("tree");
  tmpFile->cd();
  ...
  for(int i = 0; i < plots; ++i){
    ...
    //double fSig     = 0.5; //0.539; // 0.719; // 
    //double fSigUp   = fSig+0.25; // fSig*1.3; // 
    //double fSigDown = fSig-0.25; // fSig*0.7; // 
    ...
    hists[i+1*plots]->Scale(1./hists[i+1*plots]->Integral());
    ...
    TString canvName = TString("canv_comparison_")+TString(plotString[i*2](plotString[i*2].Index(">>")+3, max(plotString[i*2].Last('('), plotString[i*2].Length())-plotString[i*2].Index(">>")-3)); 
    ...
    if(i < plots){
      if(!leg){
	leg = new TLegend(0.5,0.75,0.88,0.9);
	leg->SetFillStyle(0);
	leg->SetBorderSize(0);
	//leg->AddEntry(hists[i+2*plots], "CMS data" , "p");
	leg->AddEntry(hists[i        ], "t#bar{t} background", "l");
	...
	//leg->AddEntry(histSum         , "uncertainty on f_{sig}", "f");     
      ...
      hists[i]->GetYaxis()->SetRangeUser(0, 1.1*max(hists[i]->GetMaximum(),hists[i+1*plots]->GetMaximum()));
...
void savePlotsToDisk(TString format, bool rootFile)
{
  ...
  TFile * output = TFile::Open("comparisonPlots/plots.root", "UPDATE");
  output->cd();
