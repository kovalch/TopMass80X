...
void backgroundValidationPlotsJets()
{
  TH1::SetDefaultSumw2();
  gStyle->SetOptStat(0);
  ...
  TH1F * hPt1_data = new TH1F("hPt1_data",";p_{T}^{1} (GeV);Events / (20 GeV)",20,60,460); // 23,0,460); // 
  TH1F * hPt1_sig  = (TH1F*)hPt1_data->Clone("hPt1_sig");
  TH1F * hPt1_bkg  = (TH1F*)hPt1_data->Clone("hPt1_bkg");
  TH1F * hPt2_data = new TH1F("hPt2_data",";p_{T}^{2} (GeV);Events / (10 GeV)",34,60,400); // 40,0,400); // 
  TH1F * hPt2_sig  = (TH1F*)hPt2_data->Clone("hPt2_sig");
  TH1F * hPt2_bkg  = (TH1F*)hPt2_data->Clone("hPt2_bkg");
  TH1F * hPt3_data = new TH1F("hPt3_data",";p_{T}^{3} (GeV);Events / (4 GeV)",50,60,260); // 65,0,260); // 
  TH1F * hPt3_sig  = (TH1F*)hPt3_data->Clone("hPt3_sig");
  TH1F * hPt3_bkg  = (TH1F*)hPt3_data->Clone("hPt3_bkg");
  TH1F * hPt4_data = new TH1F("hPt4_data",";p_{T}^{4} (GeV);Events / (4 GeV)",45,60,220); // 55,0,220); // 
  TH1F * hPt4_sig  = (TH1F*)hPt4_data->Clone("hPt4_sig");
  TH1F * hPt4_bkg  = (TH1F*)hPt4_data->Clone("hPt4_bkg");
  TH1F * hPt5_data = new TH1F("hPt5_data",";p_{T}^{5} (GeV);Events / (4 GeV)",30,50,170); // 40,10,170); // 
  TH1F * hPt5_sig  = (TH1F*)hPt5_data->Clone("hPt5_sig");
  TH1F * hPt5_bkg  = (TH1F*)hPt5_data->Clone("hPt5_bkg");
  TH1F * hPt6_data = new TH1F("hPt6_data",";p_{T}^{6} (GeV);Events / (4 GeV)",25,40,140); // 35,0,140); // 
  TH1F * hPt6_sig  = (TH1F*)hPt6_data->Clone("hPt6_sig");
  TH1F * hPt6_bkg  = (TH1F*)hPt6_data->Clone("hPt6_bkg");
  TH1F * hPt7_data = new TH1F("hPt7_data",";p_{T}^{7} (GeV);Events / (4 GeV)",20,30,110); // 25,0,100); //
  TH1F * hPt7_sig  = (TH1F*)hPt7_data->Clone("hPt7_sig");
  TH1F * hPt7_bkg  = (TH1F*)hPt7_data->Clone("hPt7_bkg");
  TH1F * hPt8_data = new TH1F("hPt8_data",";p_{T}^{8} (GeV);Events / (4 GeV)",15,30,90); // 20,10,90); //
  TH1F * hPt8_sig  = (TH1F*)hPt8_data->Clone("hPt8_sig");
  TH1F * hPt8_bkg  = (TH1F*)hPt8_data->Clone("hPt8_bkg");
  TH1F * hPt9_data = new TH1F("hPt9_data",";p_{T}^{9} (GeV);Events / (10 GeV)",4,30,70); // 7,0,70); //
  TH1F * hPt9_sig  = (TH1F*)hPt9_data->Clone("hPt9_sig");
  TH1F * hPt9_bkg  = (TH1F*)hPt9_data->Clone("hPt9_bkg");

  TH1F * hBRank_data = new TH1F("hBRank_data",";BRank;B Jets",9,1,10);
  TH1F * hBRank_sig  = (TH1F*)hBRank_data->Clone("hBRank_sig");
  TH1F * hBRank_bkg  = (TH1F*)hBRank_data->Clone("hBRank_bkg");
  TH1F * hBRank1_data = new TH1F("hBRank1_data",";BRank1;Events",9,1,10);
  TH1F * hBRank1_sig  = (TH1F*)hBRank1_data->Clone("hBRank_sig");
  TH1F * hBRank1_bkg  = (TH1F*)hBRank1_data->Clone("hBRank_bkg");
  ...
  for(int i = 0, Nevents = tree->GetEntries(); i < Nevents; ++i){
    tree->GetEntry(i);
    std::map<double, double> jet_pts_btags;
    std::vector<double> jet_pts(0);
    for(int j = 0; j < Njet; ++j){
      jet_pts.push_back(((TLorentzVector*)jets->At(j))->Pt());
      jet_pts_btags[((TLorentzVector*)jets->At(j))->Pt()] = bTag[j];
      if(j == 8) break;
    }
    ...
    int jetCounter = 1;
    bool filled = false;
    for(std::map<double, double>::const_reverse_iterator jet = jet_pts_btags.rbegin(); jet != jet_pts_btags.rend(); ++jet, ++jetCounter){
      if(jet->second > 0.898){
	hBRank_sig->Fill(jetCounter,combinedWeight);
	if(!filled){
	  hBRank1_sig->Fill(jetCounter,combinedWeight);
	  filled=true;
	}
      }
    }

    hPt1_sig->Fill(jet_pts.at(jet_pts.size()-1),combinedWeight);
    hPt2_sig->Fill(jet_pts.at(jet_pts.size()-2),combinedWeight);
    hPt3_sig->Fill(jet_pts.at(jet_pts.size()-3),combinedWeight);
    hPt4_sig->Fill(jet_pts.at(jet_pts.size()-4),combinedWeight);
    hPt5_sig->Fill(jet_pts.at(jet_pts.size()-5),combinedWeight);
    hPt6_sig->Fill(jet_pts.at(jet_pts.size()-6),combinedWeight);
    if(jet_pts.size()>6) hPt7_sig->Fill(jet_pts.at(jet_pts.size()-7),combinedWeight);
    if(jet_pts.size()>7) hPt8_sig->Fill(jet_pts.at(jet_pts.size()-8),combinedWeight);
    if(jet_pts.size()>8) hPt9_sig->Fill(jet_pts.at(jet_pts.size()-9),combinedWeight);
  }
  hists.push_back(hPt1_sig);
  hists.push_back(hPt2_sig);
  hists.push_back(hPt3_sig);
  hists.push_back(hPt4_sig);
  hists.push_back(hPt5_sig);
  hists.push_back(hPt6_sig);
  hists.push_back(hPt7_sig);
  hists.push_back(hPt8_sig);
  hists.push_back(hPt9_sig);
  hists.push_back(hBRank_sig);
  hists.push_back(hBRank1_sig);

  std::cout << "BRank  SIG: mean = " << hBRank_sig ->GetMean() << ", RMS = " << hBRank_sig ->GetRMS() << std::endl;
  std::cout << "BRank1 SIG: mean = " << hBRank1_sig->GetMean() << ", RMS = " << hBRank1_sig->GetRMS() << std::endl;
  ...
  for(int i = 0, Nevents = tree->GetEntries(); i < Nevents; ++i){
    ...
    for(int j = 0; j < Njet; ++j){
      //if(i < 100) std::cout << ((TLorentzVector*)jets->At(j))->Pt() << ", ";
      jet_pts.push_back(((TLorentzVector*)jets->At(j))->Pt());
      jet_pts_btags[((TLorentzVector*)jets->At(j))->Pt()] = bTag[j];
      if(j == 8) break;
    }
    ...
    for(std::map<double, double>::const_reverse_iterator jet = jet_pts_btags.rbegin(); jet != jet_pts_btags.rend(); ++jet, ++jetCounter){
      if(jet->second > 0.898){
	hBRank_bkg->Fill(jetCounter);
	if(!filled){
	  hBRank1_bkg->Fill(jetCounter);
	  filled=true;
	}
      }
    }

    hPt1_bkg->Fill(jet_pts.at(jet_pts.size()-1));
    hPt2_bkg->Fill(jet_pts.at(jet_pts.size()-2));
    hPt3_bkg->Fill(jet_pts.at(jet_pts.size()-3));
    hPt4_bkg->Fill(jet_pts.at(jet_pts.size()-4));
    hPt5_bkg->Fill(jet_pts.at(jet_pts.size()-5));
    hPt6_bkg->Fill(jet_pts.at(jet_pts.size()-6));
    if(jet_pts.size()>6) hPt7_bkg->Fill(jet_pts.at(jet_pts.size()-7));
    if(jet_pts.size()>7) hPt8_bkg->Fill(jet_pts.at(jet_pts.size()-8));
    if(jet_pts.size()>8) hPt9_bkg->Fill(jet_pts.at(jet_pts.size()-9));
  }
  hists.push_back(hPt1_bkg);
  hists.push_back(hPt2_bkg);
  hists.push_back(hPt3_bkg);
  hists.push_back(hPt4_bkg);
  hists.push_back(hPt5_bkg);
  hists.push_back(hPt6_bkg);
  hists.push_back(hPt7_bkg);
  hists.push_back(hPt8_bkg);
  hists.push_back(hPt9_bkg);
  hists.push_back(hBRank_bkg);
  hists.push_back(hBRank1_bkg);

  std::cout << "BRank  BKG: mean = " << hBRank_bkg ->GetMean() << ", RMS = " << hBRank_bkg ->GetRMS() << std::endl;
  std::cout << "BRank1 BKG: mean = " << hBRank1_bkg->GetMean() << ", RMS = " << hBRank1_bkg->GetRMS() << std::endl;
  ...
  for(int i = 0, Nevents = tree->GetEntries(); i < Nevents; ++i){
    ...
    bool filled = false;
    for(std::map<double, double>::const_reverse_iterator jet = jet_pts_btags.rbegin(); jet != jet_pts_btags.rend(); ++jet, ++jetCounter){
      //if(i < 10) std::cout << jet->first << " -> " << jet->second << std::endl;
      if(jet->second > 0.898){
	hBRank_data->Fill(jetCounter);
	if(!filled){
	  hBRank1_data->Fill(jetCounter);
	  filled=true;
	}
      }
    }

    hPt1_data->Fill(jet_pts.at(jet_pts.size()-1));
    hPt2_data->Fill(jet_pts.at(jet_pts.size()-2));
    hPt3_data->Fill(jet_pts.at(jet_pts.size()-3));
    hPt4_data->Fill(jet_pts.at(jet_pts.size()-4));
    hPt5_data->Fill(jet_pts.at(jet_pts.size()-5));
    hPt6_data->Fill(jet_pts.at(jet_pts.size()-6));
    if(jet_pts.size()>6) hPt7_data->Fill(jet_pts.at(jet_pts.size()-7));
    if(jet_pts.size()>7) hPt8_data->Fill(jet_pts.at(jet_pts.size()-8));
    if(jet_pts.size()>8) hPt9_data->Fill(jet_pts.at(jet_pts.size()-9));
  }
  hists.push_back(hPt1_data);
  hists.push_back(hPt2_data);
  hists.push_back(hPt3_data);
  hists.push_back(hPt4_data);
  hists.push_back(hPt5_data);
  hists.push_back(hPt6_data);
  hists.push_back(hPt7_data);
  hists.push_back(hPt8_data);
  hists.push_back(hPt9_data);
  hists.push_back(hBRank_data);
  hists.push_back(hBRank1_data);

  std::cout << "BRank  DATA: mean = " << hBRank_data ->GetMean() << ", RMS = " << hBRank_data ->GetRMS() << std::endl;
  std::cout << "BRank1 DATA: mean = " << hBRank1_data->GetMean() << ", RMS = " << hBRank1_data->GetRMS() << std::endl;
  ...
  for(int i = 0; i < plots; ++i){
    ...
    hists[i+1*plots]->GetYaxis()->SetTitleOffset(1.4);
    hists[i+2*plots]->GetYaxis()->SetTitleOffset(1.4);
    ...
    TString canvName = "canv_Pt"; canvName += i; //TString("canv_")+TString(plotString[i*2](plotString[i*2].Index(">>")+3, max(plotString[i*2].Last('('), plotString[i*2].Length())-plotString[i*2].Index(">>")-3)); 
