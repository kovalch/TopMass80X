...
void controlPlots_Theory()
{
  ...
  tTTUp       = new TChain("analyzeHitFit/eventTree");
  tTTDown     = new TChain("analyzeHitFit/eventTree");
  ...
  if (channel == kMuon || channel == kAll) {
    tTTJets    ->Add("/scratch/hh/current/cms/user/mseidel/Fall11_TTJets1725_v1_1.00_muon/analyzeTop.root");
    ...
    if (upDown) {
      tTTUp    ->Add("/scratch/hh/current/cms/user/mseidel/Fall11_TTJets1725_matchingup_muon/analyzeTop.root");
      tTTDown  ->Add("/scratch/hh/current/cms/user/mseidel/Fall11_TTJets1725_matchingdown_muon/analyzeTop.root");
    }
  }
  if (channel == kElectron || channel == kAll) {
    tTTJets    ->Add("/scratch/hh/current/cms/user/mseidel/Fall11_TTJets1725_v1_1.00_electron/analyzeTop.root");
    if (qcd) {
    ...
    if (upDown) {
      tTTUp    ->Add("/scratch/hh/current/cms/user/mseidel/Fall11_TTJets1725_matchingup_electron/analyzeTop.root");
      tTTDown  ->Add("/scratch/hh/current/cms/user/mseidel/Fall11_TTJets1725_matchingdown_electron/analyzeTop.root");
    }
  }
  ...
  for(unsigned int idx=0; idx<27; ++idx) {
    switch(idx) {
      ...
      case kUp: {
        crossSection = 163.; // 
        sampleSize   = 4029823.; // matching
        //sampleSize   = 930483.; // scale 
        break;
      }
      case kDown: {
        crossSection = 163.; // 
        sampleSize   = 1545688.; // matching
  ...
  makeControlPlot("event", "jetMultiplicity", "Number of jets", "", "jetMultiplicity_event", 6, 4, 10, kEvent2b);
  makeControlPlot("event", "nVertex", "Number of vertices", "", "nVertex_event", 25, 0, 25, kEvent2b);
  makeControlPlot("event", "jetMultiplicity", "Number of jets", "", "jetMultiplicity_event_log", 6, 4, 10, kEvent2b, true);
  ...
  makeControlPlot("jet", "hadBRawPt", "p_{T,b}", "GeV", "BRawPt_event_log", 40, 0, 200, kEvent2b, true);
  makeControlPlot("jet", "hadQRawPt", "p_{T,q}", "GeV", "QRawPt_event_log", 40, 0, 200, kEvent2b, true);
  makeControlPlot("jet", "hadQBarRawPt", "p_{T,#bar{q}}", "GeV", "QBarRawPt_event_log", 40, 0, 200, kEvent2b, true);
...
void makeControlPlot(TString typeForTitle, TString sObservable, TString sObservableShort, TString sUnit, TString sFileName, int nbinsx, double xlow, double xup, int plot, bool logY = false, double cut = -999) {
  ...
  if (plot == kPerm2b || plot == kPerm2bW) {
    for (int i = 0; i < hData->GetNbinsX(); i++) {
      ...
      if (upDown) {
        binError = hTTUp->GetBinError(i);
        hTTUp->SetBinError(i, binError * 2);
        
        binError = hTTDown->GetBinError(i);
        hTTDown->SetBinError(i, binError * 2);
      }
    }
  }
  ...
  if (logY) hNull->GetYaxis()->SetRangeUser(1, hData->GetMaximum()*100);
  ...
  */
  ...
  if (upDown) {
    leg0->AddEntry( hMCUp, "Matching up", "PL");
    leg0->AddEntry( hMCDown, "Matching down", "PL");
  }
  ...
  TString path("controlPlots_upDown/"); path+= sFileName; path += "_"; path += lepton_[channel]; path += ".eps";
