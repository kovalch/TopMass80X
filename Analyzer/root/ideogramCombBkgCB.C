...
void FindParameters(TString filename, int i)
{
  ...
  cb->SetParameters(1, 170, 25, 0.72, 5);
  
  cb->SetParLimits(0, 0, 1000000);
  cb->SetParLimits(1, 150, 200);
  cb->SetParLimits(2, 25, 25);
  cb->SetParLimits(3, 0.1, 10.72);
  cb->SetParLimits(4, 5, 5);
  
  TH1F* hCombBkg;
  
  eventTree->Draw("hadTopMass >> hCombBkg(80, 100, 500)","(bProb*fitProb)*((target==0 | target==-2) & bProb > 1e-3 & fitProb > 1e-3)");
