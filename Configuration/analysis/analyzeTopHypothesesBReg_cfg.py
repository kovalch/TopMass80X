...
if( hasattr(process, 'addBRegProducer' ) ):
   setForAllTtSemiLepHypotheses(process, "jets",  cms.InputTag('addBRegProducer', 'goodJetsPF30', process.name_()) )
   process.analyzeJets.jets = cms.InputTag('addBRegProducer', 'goodJetsPF30', process.name_())
   process.analyzeBRegJets.jets = cms.InputTag('addBRegProducer', 'goodJetsPF30', process.name_())
   cloneTtSemiLepEvent(process)
   #   process.hitFitTtSemiLepEventHypothesis.jets = cms.InputTag('goodJetsPF30')
   process.hitFitTtSemiLepEventHypothesis2.jesB = cms.double(-1.0)
   process.ttSemiLepHypHitFit2.useBReg = cms.bool(True)
   if not data: 
       process.ttSemiLepHypGenMatch2.useBReg = cms.bool(True)
       process.ttSemiLepHypGenMatch.useBReg = cms.bool(False)
   process.ttSemiLepHypMVADisc2.useBReg = cms.bool(True)
   process.ttSemiLepHypMVADisc.useBReg = cms.bool(False)
   from TopMass.TopEventTree.EventHypothesisAnalyzer_cfi import analyzeHypothesis
   process.analyzeBRegHitFit = analyzeHypothesis.clone(hypoClassKey = "ttSemiLepHypHitFit2:Key", ttEvent = "ttSemiLepEvent2", topBranchName = "bRegTop.", jets = cms.InputTag('addBRegProducer', 'goodJetsPF30', process.name_()) )
