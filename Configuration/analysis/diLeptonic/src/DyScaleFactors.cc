...
void DyScaleFactors::produceScaleFactors(const Samples& samples)
{
    ...
    const std::vector<std::pair<TString, TString> > v_nameStepPair = ttbar::nameStepPairs(filename, "dyScaling_Allh1_step");
                                     ...
                                     const Systematic::Systematic& systematic)const
{
    ...
    const TString step = ttbar::extractSelectionStep(fullStepname);
    if(step == ""){
