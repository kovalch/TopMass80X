...
void JetMatchAnalyzer::bookHistos(const TString& step, std::map<TString, TH1*>& m_histogram)
{
    ...
    int bins_pt           = 50;  int max_pt           = 300; //500;
...
//     int bins_eta          = 50;  int min_eta          = -5;   int max_eta    = 5;
    int bins_eta          = 50;  float min_eta        = -2.6;  float max_eta          = 2.6;
    int bins_phi          = 40;  int min_phi          = -4;    int max_phi            = 4;  //7
//     int bins_DEta         = 64;  int min_DEta         = -8;   int max_DEta   = 8;
    int bins_DEta         = 50;  int min_DEta         = -5;    int max_DEta           = 5;
    int bins_MinDEta         = 40;  int min_MinDEta   = 0;   int max_MinDEta          = 4;
//     int bins_DPhi         = 40;  int min_DPhi         = -10;  int max_DPhi   = 10;
    int bins_DPhi         = 32;  int min_DPhi         = -6;  int max_DPhi             = 6;
...
//     int bins_MinDR        = 65;  int min_MinDR        = 0;    int max_MinDR  = 0.25;    
    ...
    m_histogram[name] = this->store(new TH1D(prefix_+name+step,"Top jets;#Delta#phi_{jj};Entries",bins_DPhi,min_DPhi,max_DPhi));
   ...
   //==================================               Higgs Jets              =========================================//
    ...
    m_histogram[name] = this->store(new TH1D(prefix_+name+step,"Higgs jets;#Delta#phi_{jj};Entries",bins_DPhi,min_DPhi,max_DPhi));
                                  ...
                                  std::map<TString, TH1*>& m_histogram)
{
    ...
    //    }  
    //    if(genObjectIndices.uniqueGenTopMatching() && genObjectIndices.uniqueGenHiggsMatching()){
    ...
    //For each gen jet we find the reco jet with the minimum DeltaR!!
      for (size_t i_gen=0;i_gen!=testcommonJetIndices.size();++i_gen){  //gen
         minR = 999.;
        ...
        for(const int index : recoObjectIndices.jetIndices_){
              ...
              if (deltaR>0.4) continue; //0.5
              if ( ((commonGenObjects.allGenJets_->at(i_gen).Pt()-recoObjects.jets_->at(index).Pt())/commonGenObjects.allGenJets_->at(i_gen).Pt())<-0.4) continue; //-0.2 //-0.3  //-0.4
              if ( ((commonGenObjects.allGenJets_->at(i_gen).Pt()-recoObjects.jets_->at(index).Pt())/commonGenObjects.allGenJets_->at(i_gen).Pt())>0.6) continue; //0.4 //0.5 //0.6
    ...
    // int LeadingJetIndx =-1;
    // int SubLeadingJetIndx =-1;
    // int LeadingFlavourIndx =-1;
    // int SubLeadingFlavourIndx =-1;
    ...
    //     std::cout<<"genIndex.size()= "<<genIndex.size()<<", isInMismatchedJetsIndex.size()=  "<<isInMismatchedJetsIndex.size()<<std::endl;
