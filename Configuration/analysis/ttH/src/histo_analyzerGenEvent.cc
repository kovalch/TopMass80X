...
constexpr const char* OutputBaseDIR = "Plots_genLevel";
...
void printHistogram(TString fileName, TString legFileName, TString prefix_histname, TString ending_histname, const std::vector<Channel::Channel>& v_channel, const std::vector<Systematic::Systematic>& v_systematic){
...
int main(int argc, char** argv){
    ...
    TString prefix_histname  = "genEvent_";
    ...
    for(size_t iFileName = 0; iFileName < v_inputFileTtbar.size(); ++iFileName){
        for(const auto& iStep : v_step){
            ...
            printHistogram(fileName, legFileName , prefix_histname, ending_histname, v_channel, v_systematic);
        }
    } 
