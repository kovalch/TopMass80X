...
usePF2PAT(process, runPF2PAT=True, jetAlgo='AK5', runOnMC=options.runOnMC, postfix=pfpostfix, jetCorrections=jetCorr, pvCollection=cms.InputTag('OfflineSlimmedPrimaryVertices'), typeIMetCorrections=True)
...
''' it couldn't find mvaNonTrigV0 and it creashed when trying to create an ntuple
# MVA ID
...
# Electron ID
#getattr(process, 'selectedPatElectrons'+pfpostfix).cut = 'electronID("mvaTrigV0") > 0.5 && passConversionVeto'  # problems with mvaTrigV0
getattr(process, 'selectedPatElectrons'+pfpostfix).cut = 'electronID("eidLoose") > 0.5 && passConversionVeto' # just to check
...
process.userPatSequence = cms.Sequence(getattr(process, 'pfMuonsFromVertex'+pfpostfix))
...
correctedPatMet = "pfMetT1Txy"

'''
####################################################################
##  MVA met
''' RecoMET.METPUSubtraction.mvaPFMET_leptons_cff doesn't exist any more!!
...
path = cms.Path(
    ...
    #process.electronCorrectionSequence *  #it couldn't find RegressionEnergyPatElectronProducer/'eleRegressionEnergy' and CalibratedElectronProducer/'calibratedElectrons'
