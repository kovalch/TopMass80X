...
// ----------member data ---------------------------
    ...
    // FIXME: counters only for testing
    int counterAllHadrons_;
    int counterJetHadrons_;
};
...
counterAllHadrons_(0),
counterJetHadrons_(0)
{
...
{
    std::cout<<"\n\n\nnumber of hadrons (flavour = "<<flavour_<<"): "<<counterAllHadrons_<<" , "<<counterJetHadrons_<<"\n\n\n";
}
...
//    const unsigned int nJets = jetFlavourInfos->size();
    ...
    for(reco::GenParticleCollection::const_iterator i_particle = genParticles->begin(); i_particle != genParticles->end(); ++i_particle){
        ...
        // FIXME: do here whatever is needed for all hadrons
        std::cout<<"\n\n\n\n\thadron: "<<thisParticle->pdgId()<<"\n\n\n\n";
        
        // FIXME: This implementation does not really fit the needs for the new collections, only used for checking the counter
    ...
    // Access hadrons and leptons via jets where they are clustered to
    ...
    for(reco::JetFlavourInfoMatchingCollection::const_iterator i_info  = jetFlavourInfos->begin(); i_info != jetFlavourInfos->end(); ++i_info){
        const reco::Jet* thisJet = i_info->first.get();
        reco::JetFlavourInfo thisInfo = i_info->second;
        const reco::GenParticleRefVector& bHadrons = thisInfo.getbHadrons();
        const reco::GenParticleRefVector& cHadrons = thisInfo.getcHadrons();
        const reco::GenParticleRefVector& leptons = thisInfo.getLeptons();
        // FIXME: do here whatever is needed for hadrons in this jet
        std::cout<<"\njet: "<<thisJet->pt()<<" , "<<bHadrons.size()<<" , "<<cHadrons.size()<<" , "<<leptons.size()<<"\n\n";
        const reco::GenParticleRefVector& hadrons = flavour_==5 ? bHadrons : flavour_==4 ? cHadrons : reco::GenParticleRefVector();
        counterJetHadrons_ += hadrons.size();
    }
