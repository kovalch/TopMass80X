...
bWeightSrc_bTagCjetSFUp_    (cfg.getParameter<edm::InputTag>("bWeightSrc_bTagCjetSFUp")),
bWeightSrc_bTagCjetSFDown_  (cfg.getParameter<edm::InputTag>("bWeightSrc_bTagCjetSFDown")),
...
lepIDWeightSrcDown_ (cfg.getParameter<edm::InputTag>("lepIDWeightSrcDown")),
...
triggerWeightSrcUp_   (cfg.getParameter<edm::InputTag>("triggerWeightSrcUp")),
triggerWeightSrcDown_ (cfg.getParameter<edm::InputTag>("triggerWeightSrcDown")),
...
showLHEweightTypes2_(cfg.getParameter<bool>("showLHEweightTypes")),
weight(0)
{	
  ...
  mayConsume<LHERunInfoProduct, edm::BranchType::InRun>(lheEventSrc_);
...
WeightEventAnalyzer::analyze(const edm::Event& evt, const edm::EventSetup& setup)
{
  ...
  if(genEventInfo_h.isValid()){
    ...
    if(savePDFWeights_){
      ...
      weight->x1  = genEventInfo_h->pdf()->x.first;
       std::cout << "genEventInfo_h->pdf()->x.first = "<<genEventInfo_h->pdf()->x.first << std::endl;
  ...
  if(lheEvent_h.isValid()){
    ...
    if (showLHEweightTypes_) {
      std::cout << "\n comments:\n";
      for (unsigned int i = 0; i < lheEvent_h->comments_size(); ++i) {
        std::cout << lheEvent_h->getComment(i) << std::endl;
      }
  ...
  if (!ttEvent) {
    ...
    edm::LogWarning("noTTEvent") << "no ttEvent is here";
  } else if (ttEvent->genEvent().isNull() ||
             (!ttEvent->genEvent()->isFullHadronic() &&
              !ttEvent->genEvent()->isSemiLeptonic() &&
              !ttEvent->genEvent()->isFullLeptonic())) {
    weight->topPtSysWeight = 1;
  } else {
    if (std::abs(topPtSF_[0] * topPtSF_[1] * topPtSF_[2]) > 0) {
      if (ttEvent->genEvent()->top()->pt() < topPtSF_[2]) {
        if (ttEvent->genEvent()->topBar()->pt() < topPtSF_[2]) {
          weight->topPtSysWeight =
              sqrt(exp(topPtSF_[0] +
                       topPtSF_[1] * ttEvent->genEvent()->top()->pt()) *
                   exp(topPtSF_[0] +
                       topPtSF_[1] * ttEvent->genEvent()->topBar()->pt()));
        } else {
          weight->topPtSysWeight = sqrt(exp(
              topPtSF_[0] + topPtSF_[1] * ttEvent->genEvent()->top()->pt()));
        }

      } else if (ttEvent->genEvent()->topBar()->pt() < topPtSF_[2]) {
        weight->topPtSysWeight = sqrt(exp(
            topPtSF_[0] + topPtSF_[1] * ttEvent->genEvent()->topBar()->pt()));
      } else {
        weight->topPtSysWeight = 1;
      }
    }
  ...
  if(bWeight_bTagCjetSFUp_h    .isValid())   weight->bTagWeight_bTagCjetSFUp     = *bWeight_bTagCjetSFUp_h    ;
  if(bWeight_bTagCjetSFDown_h  .isValid())   weight->bTagWeight_bTagCjetSFDown   = *bWeight_bTagCjetSFDown_h  ;
...
//   if(triggerWeight_h    .isValid()) { weight->triggerWeight 	= (*triggerWeight_h!=0.0?*triggerWeight_h:1.0); weight->combinedWeight *= weight->triggerWeight; }
  if(lepIDWeight_h    .isValid()) { weight->lepIDWeight     = *lepIDWeight_h     	; weight->combinedWeight *= weight->lepIDWeight; }
...
//   if(triggerWeight_h    .isValid()) { weight->triggerWeight 	= (*triggerWeight_h!=0.0?*triggerWeight_h:1.0); weight->combinedWeight *= weight->triggerWeight; }
  if(isoWeight_h    .isValid()) { weight->isoWeight     = *isoWeight_h     	; weight->combinedWeight *= weight->isoWeight; }
...
//   if(triggerWeight_h    .isValid()) { weight->triggerWeight 	= (*triggerWeight_h!=0.0?*triggerWeight_h:1.0); weight->combinedWeight *= weight->triggerWeight; }
  ...
  if(triggerWeightDown_h.isValid())   weight->triggerWeightDown = *triggerWeightDown_h	; 
...
void WeightEventAnalyzer::endRun(const edm::Run& irun, const edm::EventSetup& setup) {
  // mayConsume<LHERunInfoProduct>(lheEventSrc_);
  if (showLHEweightTypes2_) {

    std::cout << "\n LHERunInfoProduct tags and lines:\n";

    edm::Handle<LHERunInfoProduct> LHERunInfoProduct_h;
    // typedef std::vector<LHERunInfoProduct::Header>::const_iterator
    //   headers_const_iterator;

    irun.getByLabel(lheEventSrc_, LHERunInfoProduct_h);
    // evt.getByLabel("externalLHEProducer", run);
    // LHERunInfoProduct myLHERunInfoProduct =
    // *(LHERunInfoProduct_h.product());

    for (std::vector<LHERunInfoProduct::Header>::const_iterator iter =
             LHERunInfoProduct_h->headers_begin();
         iter != LHERunInfoProduct_h->headers_end(); iter++) {
      std::cout << "\n LHERunInfoProduct tag:\n";
