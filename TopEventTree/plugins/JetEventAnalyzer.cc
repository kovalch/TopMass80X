...
JetEventAnalyzer::JetEventAnalyzer(const edm::ParameterSet &cfg)
    : jets_(consumes<std::vector<pat::Jet> >(
      ...
      met_(consumes<std::vector<pat::MET> >(
          cfg.getParameter<edm::InputTag>("met"))),
      gluonTagName_(cfg.getParameter<edm::InputTag>("gluonTagSrc").encode()),
      kJetMAX_(cfg.getParameter<int>("maxNJets")),
      jet(0),
      checkedIsPFJet(false),
      checkedJERSF(false),
      checkedJESSF(false),
      checkedTotalSF(false),
      checkedQGTag(false),
      checkedBReg(false),
      isPFJet(false),
      hasJERSF(false),
...
void JetEventAnalyzer::analyze(const edm::Event &evt,
  ...
  ////////////////////////////////////////////////////////////////////////
  // instantiate a tagging variable computer for unification of some
  // calculations like vertex mass corrections
  ////////////////////////////////////////////////////////////////////////
  // edm::ESHandle<JetTagComputer> computerHandle;;
  // setup.get<JetTagComputerRecord>().get( "combinedSecondaryVertex",
  // computerHandle );
  // const GenericMVAJetTagComputer *computer = dynamic_cast<const
  // GenericMVAJetTagComputer*>( computerHandle.product() );
  // if (!computer){
  ...
  for (std::vector<pat::Jet>::const_iterator ijet = jets->begin();
    ...
    // Control the jet order
    if (ijet != jets->begin() && ijet->pt() > (ijet - 1)->pt())
      throw cms::Exception("WrongJetOrder")
          << "ERROR: Jets are not in pt order! \n";
    jet->jet.push_back(
        TLorentzVector(ijet->px(), ijet->py(), ijet->pz(), ijet->energy()));
    if (ijet->genJet())
      jet->genJet.push_back(
          TLorentzVector(ijet->genJet()->px(), ijet->genJet()->py(),
                         ijet->genJet()->pz(), ijet->genJet()->energy()));
    else
      jet->genJet.push_back(TLorentzVector(0, 0, 0, -1));
    if (ijet->genParton())
      jet->genParton.push_back(
          TLorentzVector(ijet->genParton()->px(), ijet->genParton()->py(),
                         ijet->genParton()->pz(), ijet->genParton()->energy()));
    else
      jet->genParton.push_back(TLorentzVector(0, 0, 0, -1));

    // check only once per module run if the needed collections are available
    if (!checkedIsPFJet) {
      checkedIsPFJet = true;
      isPFJet = ijet->isPFJet();
    }
    if (!checkedQGTag) {
      checkedQGTag = true;
      if (ijet->hasUserFloat(gluonTagName_)) hasQGTag = true;
    }
    if (!checkedJERSF) {
      checkedJERSF = true;
      if (ijet->hasUserFloat("jerSF")) hasJERSF = true;
    }
    if (!checkedJESSF) {
      checkedJESSF = true;
      if (ijet->hasUserFloat("jesSF")) hasJESSF = true;
    }
    if (!checkedTotalSF) {
      checkedTotalSF = true;
      if (ijet->hasUserFloat("totalSF")) hasTotalSF = true;
    }
    if (!checkedBReg) {
      checkedBReg = true;
      if (ijet->hasUserFloat("BRegResult")) hasBReg = true;
    }
    ...
    if (svTagInfo && svTagInfo->nVertices() > 0) {
      ...
      // baseTagInfos.push_back(
      // ijet->tagInfoSecondaryVertex("secondaryVertex"));
      ...
      // svmom.SetPtEtaPhiM(vertex.p4().pt(), vertex.p4().eta(),
      // vertex.p4().phi(), vars.get(reco::btau::vertexMass));
      //}
  ...
  if (alternativeJetsAvailable) {
    ...
    for (const auto &altJet : *alternativeJets) {
      jet->alternativeJet.push_back(TLorentzVector(
          altJet.px(), altJet.py(), altJet.pz(), altJet.energy()));
  ...
  if (met.isValid()) {
    jet->met = TLorentzVector(met->at(0).px(), met->at(0).py(), met->at(0).pz(),
                              met->at(0).energy());
...
void JetEventAnalyzer::beginJob() {
  if (!trs)
...
void JetEventAnalyzer::endJob() {}

JetEventAnalyzer::~JetEventAnalyzer() {}

std::pair<TVector2, TVector2> JetEventAnalyzer::getPullVector(
    std::vector<pat::Jet>::const_iterator patJet) {
  TVector2 null(0, 0);
  ...
  for (size_t idx = 0, length = patJet->numberOfDaughters(); idx < length;
       ++idx) {
    const reco::Candidate *cand = patJet->daughter(idx);
    if (cand->charge() != 0) {
      constituent.SetPtEtaPhiE(cand->pt(), cand->eta(), cand->phi(),
                               cand->energy());
  ...
  double jetPt = patJet->pt(), jetPhi = patJet->phi(),
         jetRapidity = patJet->rapidity();
  double jetPtCharged = chargedJet.Pt(), jetPhiCharged = chargedJet.Phi(),
         jetRapidityCharged = chargedJet.Rapidity();
  TVector2 r(0, 0);
  ...
  for (size_t idx = 0, length = patJet->numberOfDaughters(); idx < length;
    ...
    r.Set(constituentRapidity - jetRapidity,
          TVector2::Phi_mpi_pi(constituentPhi - jetPhi));
    pullAll += (constituentPt / jetPt) * r.Mod() * r;
    // calculate TVector using only charged tracks
    if (cand->charge() != 0)
      r.Set(constituentRapidity - jetRapidityCharged,
            TVector2::Phi_mpi_pi(constituentPhi - jetPhiCharged));
    pullCharged += (constituentPt / jetPtCharged) * r.Mod() * r;
  }

  // if there are less than two charged tracks do not calculate the pull (there
  // is not enough info), return null vector
  // TODO really needed???????
  if (nCharged < 2) pullCharged = null;

  return std::make_pair(pullAll, pullCharged);
}

// TODO Sorry for duplicating this function. Maybe there is a nicer option?
std::pair<TVector2, TVector2> JetEventAnalyzer::getGenPullVector(
    std::vector<pat::Jet>::const_iterator patJet) {
  TVector2 null(0, 0);
  ...
  for (size_t idx = 0, length = patJet->genJet()->numberOfDaughters();
       idx < length; ++idx) {
  ...
  double jetPt = patJet->genJet()->pt(), jetPhi = patJet->genJet()->phi(),
         jetRapidity = patJet->genJet()->rapidity();
  double jetPtCharged = chargedJet.Pt(), jetPhiCharged = chargedJet.Phi(),
         jetRapidityCharged = chargedJet.Rapidity();
  TVector2 r(0, 0);
  TVector2 pullAll(0, 0);
