// -*- C++ -*-
//
// Package:    GenHFHadronMatcher_JetFlavourInput
// Class:      GenHFHadronMatcher_JetFlavourInput
//

/**\class GenHFHadronMatcher_JetFlavourInput GenHFHadronMatcher_JetFlavourInput.cc
...
*
* The description of the products can be found at GenHFHadronMatcher_JetFlavourInput()
*/
...
class GenHFHadronMatcher_JetFlavourInput : public edm::EDProducer
{
public:
    explicit GenHFHadronMatcher_JetFlavourInput ( const edm::ParameterSet& );
    ~GenHFHadronMatcher_JetFlavourInput();
...
GenHFHadronMatcher_JetFlavourInput::GenHFHadronMatcher_JetFlavourInput ( const edm::ParameterSet& cfg ):
...
jetFlavourInfosToken_(consumes<reco::JetFlavourInfoMatchingCollection>(cfg.getParameter<edm::InputTag>("jetFlavourInfos")))
{
    ...
    if ( flavour_==5 ) {
        ...
        edm::LogError ( "GenHFHadronMatcher_JetFlavourInput" ) << "Flavour option must be 4 (c-jet) or 5 (b-jet), but is: " << flavour_ << ". Correct this!";
    }
...
GenHFHadronMatcher_JetFlavourInput::~GenHFHadronMatcher_JetFlavourInput()
{
}
...
*
*/
void GenHFHadronMatcher_JetFlavourInput::fillDescriptions ( edm::ConfigurationDescriptions& descriptions )
{
...
void GenHFHadronMatcher_JetFlavourInput::produce ( edm::Event& evt, const edm::EventSetup& setup )
{
...
//     printf("Run: %d\tLumi: %d\tEvent: %d\n", evt.id().run(), evt.luminosityBlock(), evt.id().event());
    ...
    *hadJetIndex = findHadronJets (genParticles.product(), jetFlavourInfos.product(), *hadIndex, *hadMothers, *hadMothersIndices, *hadLeptonIndex, *hadLeptonHadIndex, *hadFlavour, *hadFromTopWeakDecay, *hadBHadronId );
...
void GenHFHadronMatcher_JetFlavourInput::beginJob()
{
}

// ------------ method called once each job just after ending the event loop  ------------
void GenHFHadronMatcher_JetFlavourInput::endJob()
{
}

// ------------ method called when starting to processes a run  ------------
void GenHFHadronMatcher_JetFlavourInput::beginRun ( edm::Run&, edm::EventSetup const& )
{
}

// ------------ method called when ending the processing of a run  ------------
void
GenHFHadronMatcher_JetFlavourInput::endRun ( edm::Run&, edm::EventSetup const& )
{
}

// ------------ method called when starting to processes a luminosity block  ------------
void GenHFHadronMatcher_JetFlavourInput::beginLuminosityBlock ( edm::LuminosityBlock&, edm::EventSetup const& )
{
}

// ------------ method called when ending the processing of a luminosity block  ------------
void GenHFHadronMatcher_JetFlavourInput::endLuminosityBlock ( edm::LuminosityBlock&, edm::EventSetup const& )
{
}
...
*/
std::vector<int> GenHFHadronMatcher_JetFlavourInput::findHadronJets ( const reco::GenParticleCollection* genParticles, const reco::JetFlavourInfoMatchingCollection* jetFlavourInfos,
...
//    const unsigned int nJets = jetFlavourInfos->size();
    int topDaughterQId = -1;
    int topBarDaughterQId= -1;
    
    // Access all hadrons, independent of their clustering to jets
    for(reco::GenParticleCollection::const_iterator i_particle = genParticles->begin(); i_particle != genParticles->end(); ++i_particle){
        ...
        if(!this->isHadron(flavour_, thisParticle)) continue;
        
        // FIXME: Leptons are not found since scanning starts directly from hadrons
        // Should start from final state particles (jet constituents or packedGenParticles) to find leptons on its way to hadrons
    ...
    for ( unsigned int hadNum=0; hadNum<nHad; hadNum++ ) {
        ...
        // Finding a jet to which the current hadron has been clustered
        // Most important info source is:
        // PhysicsTools/JetExamples/test/printJetFlavourInfo.cc
        // PhysicsTools/JetExamples/test/printJetFlavourInfo.py
        for(reco::JetFlavourInfoMatchingCollection::const_iterator i_info  = jetFlavourInfos->begin(); i_info != jetFlavourInfos->end(); ++i_info){
            ...
            for(reco::GenParticleRefVector::const_iterator hadronInJet = hadronsInJet.begin(); hadronInJet != hadronsInJet.end(); ++hadronInJet) {
                if(hadron->pdgId() != (*hadronInJet)->pdgId()) continue;
                if(std::fabs(hadron->eta() - (*hadronInJet)->eta()) > 0.00001) continue;
                if(std::fabs(hadron->phi() - (*hadronInJet)->phi()) > 0.00001) continue;
                jetIndex=i_info - jetFlavourInfos->begin();
                break;
            }       // End of loop over hadrons injected to the jet
            if(jetIndex >= 0) break;
...
*/
int GenHFHadronMatcher_JetFlavourInput::isInList ( std::vector<const reco::Candidate*> particleList, const reco::Candidate* particle )
{
...
int GenHFHadronMatcher_JetFlavourInput::isInList ( std::vector<int> list, const int value )
{
...
*/
bool GenHFHadronMatcher_JetFlavourInput::isHadron ( const int flavour, const reco::Candidate* thisParticle )
{
...
*/
bool GenHFHadronMatcher_JetFlavourInput::isHadronPdgId ( const int flavour, const int pdgId )
{
...
*/
bool GenHFHadronMatcher_JetFlavourInput::hasHadronDaughter ( const int flavour, const reco::Candidate* thisParticle )
{
...
*/
int GenHFHadronMatcher_JetFlavourInput::analyzeMothers ( const reco::Candidate* thisParticle, pCRC *hadron, pCRC *lepton, int& topDaughterQId, int& topBarDaughterQId, std::vector<const reco::Candidate*> &hadMothers, std::vector<std::vector<int> > &hadMothersIndices, std::set<const reco::Candidate*> *analyzedParticles, const int prevPartIndex )
{
...
*/
bool GenHFHadronMatcher_JetFlavourInput::putMotherIndex ( std::vector<std::vector<int> > &hadMothersIndices, int partIndex, int mothIndex )
{
...
int GenHFHadronMatcher_JetFlavourInput::findInMothers ( int idx, std::vector<int> &mothChains, std::vector<std::vector<int> > &hadMothersIndices, std::vector<reco::GenParticle> &hadMothers, int status, int pdgId, bool pdgAbs=false, int stopId=-1, int firstLast=0, bool verbose=false	)
{
    int foundStopId = -1;
...
*/
bool GenHFHadronMatcher_JetFlavourInput::isNeutralPdg ( int pdgId )
{
    const int max = 5;
 ...
 */
bool GenHFHadronMatcher_JetFlavourInput::fixExtraSameFlavours(
...
DEFINE_FWK_MODULE ( GenHFHadronMatcher_JetFlavourInput );
