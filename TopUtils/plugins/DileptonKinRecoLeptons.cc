...
/*    
    edm::Handle<std::vector<std::pair<const reco::RecoCandidate*, const reco::RecoCandidate*> > > dileptons;
    iEvent.getByLabel(dileptons_, dileptons);
    
    // Find the leading lepton
    const reco::RecoCandidate* leadingLepton(0);
    for(std::vector<std::pair<const reco::RecoCandidate*, const reco::RecoCandidate*> >::const_iterator i_dilepton = dileptons->begin(); i_dilepton != dileptons->end(); ++i_dilepton){
        const reco::RecoCandidate* lepton(i_dilepton->first);
        const double pt = lepton->pt();
        if(!leadingLepton) leadingLepton = lepton;
        else if(pt > leadingLepton->pt()) leadingLepton = lepton;
    }
    
    // Find the subleading lepton (several combinations might exist for the leading lepton)
    const reco::RecoCandidate* subleadingLepton(0);
    for(std::vector<std::pair<const reco::RecoCandidate*, const reco::RecoCandidate*> >::const_iterator i_dilepton = dileptons->begin(); i_dilepton != dileptons->end(); ++i_dilepton){
        if(leadingLepton != i_dilepton->first) continue;
        const reco::RecoCandidate* lepton(i_dilepton->second);
        const double pt = lepton->pt();
        if(!subleadingLepton) subleadingLepton = lepton;
        else if(pt > subleadingLepton->pt()) subleadingLepton = lepton;
    }
...
/*    
    // Access leading electron and antielecton
    const reco::RecoCandidate* leadingElectron(0);
    const reco::RecoCandidate* leadingAntielecton(0);
    if(filterChannel_ != mumu){
        edm::Handle<std::vector<pat::Electron> > electrons;
        iEvent.getByLabel(electrons_, electrons);
        for(std::vector<pat::Electron>::const_iterator i_lepton = electrons->begin(); i_lepton != electrons->end(); ++i_lepton){
            if(i_lepton->charge() == -1) leadingElectron = &*i_lepton;
            if(i_lepton->charge() == 1) leadingAntielecton = &*i_lepton;
            if(leadingElectron && leadingAntielecton) break;
        }
    }
    
    // Access leading muon and antimuon
    const reco::RecoCandidate* leadingMuon(0);
    const reco::RecoCandidate* leadingAntimuon(0);
    if(filterChannel_ != ee){
        edm::Handle<std::vector<pat::Muon> > muons;
        iEvent.getByLabel(muons_, muons);
        for(std::vector<pat::Muon>::const_iterator i_lepton = muons->begin(); i_lepton != muons->end(); ++i_lepton){
            if(i_lepton->charge() == -1) leadingMuon = &*i_lepton;
            if(i_lepton->charge() == 1) leadingAntimuon = &*i_lepton;
            if(leadingMuon && leadingAntimuon) break;
        }
    }
    ...
    if(filterChannel_ == ee){
        if(leadingElectron && leadingAntielecton){
            selectedElectrons->push_back(* static_cast<const pat::Electron*>(leadingElectron));
            selectedElectrons->push_back(* static_cast<const pat::Electron*>(leadingAntielecton));
        }
    }
    else if(filterChannel_ == mumu){
        if(leadingMuon && leadingAntimuon){
            selectedMuons->push_back(* static_cast<const pat::Muon*>(leadingMuon));
            selectedMuons->push_back(* static_cast<const pat::Muon*>(leadingAntimuon));
        }
    }
    else{
        const double ptElectron = leadingElectron ? leadingElectron->pt() : -999.;
        const double ptAntielectron = leadingAntielecton ? leadingAntielecton->pt() : -999.;
        const double ptMuon = leadingMuon ? leadingMuon->pt() : -999.;
        const double ptAntimuon = leadingAntimuon ? leadingAntimuon->pt() : -999.;
        
        const reco::RecoCandidate* leadingLepton(0);
        const reco::RecoCandidate* leadingAntilepton(0);
        if(filterChannel_ == emu){
            if(leadingElectron && leadingAntimuon && ptElectron>ptMuon && ptAntielectron<ptAntimuon){
                selectedElectrons->push_back(* static_cast<const pat::Electron*>(leadingElectron));
                selectedMuons->push_back(* static_cast<const pat::Muon*>(leadingAntimuon));
            }
            else if(leadingAntielecton && leadingMuon && ptElectron<ptMuon && ptAntielectron>ptAntimuon){
                selectedElectrons->push_back(* static_cast<const pat::Electron*>(leadingAntielecton));
                selectedMuons->push_back(* static_cast<const pat::Muon*>(leadingMuon));
            }
        }
        else if(filterChannel_ == undefined){
