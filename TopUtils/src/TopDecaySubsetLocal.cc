...
#include "TopAnalysis/TopUtils/interface/TopDecaySubsetLocal.h"

// #include "FWCore/Common/interface/Provenance.h"

/// default constructor
TopDecaySubsetLocal::TopDecaySubsetLocal(const edm::ParameterSet& cfg):
  srcToken_         (cfg.getParameter<edm::InputTag>("src")),
  genEventInfo_srcToken_ (edm::InputTag("generator")),
  addRadiation_(cfg.getParameter<bool>("addRadiation")),
  showerModel_ (kStart),
  runMode_(kRun1)
{
  // mapping of the corresponding fillMode; see FillMode
  // enumerator of TopDecaySubsetLocal for available modes
...
/// default destructor
TopDecaySubsetLocal::~TopDecaySubsetLocal()
{
}

/// write output into the event
void
TopDecaySubsetLocal::produce(edm::Event& event, const edm::EventSetup& setup)
{
  ...
  event.getByLabel(srcToken_, src);
...
TopDecaySubsetLocal::findTops(const reco::GenParticleCollection& parts)
{
  std::vector<const reco::GenParticle*> tops;
...
std::vector<const reco::GenParticle*> TopDecaySubsetLocal::findPrimalTops(
...
std::vector<const reco::GenParticle*> TopDecaySubsetLocal::findDecayingTops(
		const reco::GenParticleCollection& parts) {
	std::vector<const reco::GenParticle*> tops;
	for (reco::GenParticleCollection::const_iterator t = parts.begin();
			t != parts.end(); ++t) {
...
const reco::GenParticle* TopDecaySubsetLocal::findPrimalW(
...
//const reco::GenParticle* TopDecaySubsetLocal::findDecayingW(
//		const reco::GenParticle* top) {
...
const reco::GenParticle* TopDecaySubsetLocal::findLastParticleInChain(
...
TopDecaySubsetLocal::ShowerModel
TopDecaySubsetLocal::checkShowerModel(const std::vector<const reco::GenParticle*>& tops) const
{
...
TopDecaySubsetLocal::ShowerModel TopDecaySubsetLocal::checkShowerModel(
		edm::Event& event) {
	edm::Handle < GenEventInfoProduct > genEvtInfoProduct;
	event.getByLabel(genEventInfo_srcToken_, genEvtInfoProduct);
...
void
TopDecaySubsetLocal::checkWBosons(std::vector<const reco::GenParticle*>& tops) const
{
...
void
TopDecaySubsetLocal::fillListing(const std::vector<const reco::GenParticle*>& tops, reco::GenParticleCollection& target)
{
...
void TopDecaySubsetLocal::fillListing(
...
TopDecaySubsetLocal::p4(const std::vector<const reco::GenParticle*>::const_iterator top, int statusFlag)
{
...
reco::Particle::LorentzVector
TopDecaySubsetLocal::p4(const reco::GenParticle::const_iterator part, int statusFlag)
{
...
void
TopDecaySubsetLocal::addRadiation(int& idx, const reco::GenParticle::const_iterator part, reco::GenParticleCollection& target)
{
...
void TopDecaySubsetLocal::addRadiation(int& idx, const reco::GenParticle* part,
		reco::GenParticleCollection& target) {
	std::vector<int> daughters;
	int idxBuffer = idx;
...
void
TopDecaySubsetLocal::addDaughters(int& idx, const reco::GenParticle::const_iterator part, reco::GenParticleCollection& target, bool recursive)
{
  std::vector<int> daughters;
  int idxBuffer = idx;
  for(reco::GenParticle::const_iterator daughter=part->begin(); daughter!=part->end(); ++daughter){
...
/// clear references
void
TopDecaySubsetLocal::clearReferences()
{
...
void
TopDecaySubsetLocal::fillReferences(const reco::GenParticleRefProd& ref, reco::GenParticleCollection& sel)
{
  int idx=0;
...
DEFINE_FWK_MODULE(TopDecaySubsetLocal);